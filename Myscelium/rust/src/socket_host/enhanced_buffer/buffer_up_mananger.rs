...
pub fn buffer_up_remove_schedule_by_id(id: i32) {
    let conn = BUFFER_POOL.get_connection().unwrap();
    let result = conn.execute("DELETE from CommandsTosend where ID = ?", params![id]);
...
pub fn buffer_up_remove_schedule_by_parity_id(client_id: String, parity_id: String) {
    let conn = BUFFER_POOL.get_connection().unwrap();
    let result = conn.execute("DELETE from CommandsTosend where ClientID = ? AND ParityId = ?", params![client_id, parity_id]);
    ...
    match result {
        ...
        Err(e) => {
            eprintln!(
                "An error occurred while removing scheduled command of parity_id: {} from client: {} in the CommandsTosend table: {}",
