...
pub fn client_mananger_initialize_table(buffer_path: String) {
    ...
    with_connection!(BUFFER_POOL, |conn: &rusqlite::Connection| {
        let result = conn.execute(
            "CREATE TABLE IF NOT EXISTS ClientCommandsTosend (ID INT PRIMARY KEY, ClientKey TEXT, PermissionGroup TEXT, PermissionLevel NUMBER, SuperUser BOOL, LastContact NUMBER, SubChanelsAvaliable NUMBER, SubChannelsInUse NUMBER, Streaming BOOL)",
            params![],
        );
